...
import imgCatCh2 from "../../imports/ch2.png";
import imgCatCh3 from "../../imports/ch3.png";
import imgCatCh4 from "../../imports/ch4.png";
import imgCatCh5 from "../../imports/ch5.png";
import imgCatCh6 from "../../imports/ch6.png";
import imgCatCh7 from "../../imports/ch7.png";
import imgCatCh8 from "../../imports/ch8.png";
import imgCatCh9 from "../../imports/ch9.png";
import imgCatCh10 from "../../imports/ch10.png";
import imgCatCh11 from "../../imports/ch11.png";
...
function getRecordCat(memoCount: number) {
  if (memoCount === 0) return imgCat1Photoroom2;
  return RECORD_CATS[Math.min(Math.floor(memoCount / 2), RECORD_CATS.length - 1)];
}
    ...
    <div className="bg-white flex-[1_0_0] min-h-px relative w-full" data-name="p">
      <div className="flex flex-row items-center justify-center size-full">
        <div className="content-stretch flex items-center justify-center pl-[14px] relative size-full">
          <p className="font-['Pretendard:Medium',sans-serif] leading-none not-italic relative top-[1px] shrink-0 text-[#1e293b] text-[16px] text-center whitespace-nowrap">{memoCount} / 100</p>
        </div>
      </div>
    </div>
  );
...
function Li({ memoCount }: { memoCount: number }) {
  return (
    <div className="absolute h-[22px] left-[60px] rounded-[4px] top-[22px] w-[90px] border-2 border-[#1e293b] shadow-[4px_4px_0_#1e293b]" data-name="li">
...
function Myname({ userName }: { userName: string }) {
  return (
    <div className="bg-white h-[22px] relative rounded-[4px] shrink-0 w-full border-2 border-[#1e293b] shadow-[4px_4px_0_#1e293b]" data-name="myname">
      <div className="flex flex-col items-center justify-center overflow-clip rounded-[inherit] size-full">
        <div className="content-stretch flex flex-col items-center justify-center pr-[10px] relative size-full">
          <p className="font-['Pretendard:Medium',sans-serif] leading-none not-italic relative shrink-0 text-[#1e293b] text-[16px] whitespace-nowrap">{userName || '슈빈'}</p>
        </div>
      </div>
    </div>
  );
...
function Catname({ catName }: { catName: string }) {
  return (
    <div className="bg-white h-[18px] relative rounded-[4px] shrink-0 w-[70px] border-2 border-[#1e293b] shadow-[4px_4px_0_#1e293b]" data-name="catname">
      <div className="content-stretch flex flex-col items-center justify-center overflow-clip pr-[10px] relative rounded-[inherit] size-full">
        <p className="font-['Pretendard:Medium',sans-serif] leading-none not-italic relative shrink-0 text-[#1e293b] text-[14px] whitespace-nowrap">{catName || '고영희'}</p>
      </div>
    </div>
  );
}
...
function Btn({ onProfileClick }: { onProfileClick: () => void }) {
  return (
    <button onClick={onProfileClick} className="bg-white content-stretch flex items-center justify-center mr-[-15px] relative rounded-[4px] shrink-0 size-[62px] border-2 border-[#1e293b] shadow-[4px_4px_0_#1e293b]" data-name="btn">
      <Me />
    </button>
  );
}
  ...
  return (
    <div className="bg-[#fffbf2] content-stretch flex flex-col gap-[8px] items-center overflow-clip relative rounded-[49px] size-full" data-name="main">
      ...
      <div className="absolute content-stretch flex items-center left-[153px] size-[84px] top-[483px]" data-name="cheesecat">
        <div className="flex-[1_0_0] h-full min-w-px relative" data-name="cat1-Photoroom 2">
          <img alt="" className="absolute inset-0 max-w-none object-contain pointer-events-none size-full" src={getRecordCat(memoCount)} />
        </div>
      </div>
    </div>
  );
